<template>
  <v-container fluid>
    ...
    <v-row>
      <v-col v-show="!showDrag" cols="12" xs="12" sm="12" md="6" lg="8">
        <v-card>
          <v-card-text>
            <v-row>
              <v-col v-show="!showDrag" cols="12" sm="12" md="12">
                <v-file-input
                  outlined
                  v-model="fileUpload"
                  label="Berkas PDF"
      ...
      <v-col v-if="showDrag" cols="12" xs="12" sm="12" md="6" lg="8">
        <v-card>
          <v-card-text class="example-drag">
            <div class="text-center p-5">
              <h4>
                Tarik dan jatuhkan berkas PDF di area ini untuk mengunggah
                <br />atau
              </h4>
              ...
              <file-upload
                ...
                ref="upload"
              >
                <v-btn color="success darken-1" small>Pilih berkas PDF</v-btn>
              </file-upload>
            </div>
            <div v-show="$refs.upload && $refs.upload.dropActive" class="drop-active">
              <h3>Jatuhkan berkas PDF</h3>
            </div>
          </v-card-text>
        </v-card>
      </v-col>
...
export default {
  ...
  data() {
    return {
      ...
      //     !value ||
      //     value.size < 10000000 ||
      //     "Ukuran berkas harus lebih kecil dari 10 MB!",
      // ],
        ...
        if (localStorage.hasOwnProperty("authPage")) {
          ...
          if (listPage.includes("esign")) {
            ...
            this.$axios
              .get("/esign/image",
              {
                params: { npk:this.npk, pps: this.inPps, kodeKantor:this.kodeKantorUser } ,
                responseType: "arraybuffer"
              })
              .then((response) => {
                let b64 = Buffer.from(response.data, "binary").toString("base64");
                this.imageTtdBase64 = b64;
                console.log('testtt1',this.imageTtdBase64 )
