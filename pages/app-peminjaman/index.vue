      ...
      <v-col v-show="isLoaded">
        ...
        <template v-slot:item="{ item, index }">
            <tr>
              ...
              <td v-if="item.stApproval==='Y'">{{ item.namaPeminjam==null?'-':item.namaPeminjam }}</td> <td v-else class="font-weight-bold">{{ item.namaPeminjam==null?'-':item.namaPeminjam }}</td>
              <td v-if="item.stApproval==='Y'">{{ item.kodeKantorPeminjam }}</td><td v-else class="font-weight-bold">{{ item.kodeKantorPeminjam }}</td>
              <td v-if="item.stApproval==='Y'">{{ item.tglPeminjaman==null?'-':item.tglPeminjaman}}</td><td v-else class="font-weight-bold">{{ item.tglPeminjaman==null?'-':item.tglPeminjaman}}</td>
              <td>
                <v-dialog
                    v-model="menu[index]"
                    :close-on-content-click="false"
                  ...
                  >
                    ...
                    <peminjaman-status-view-app :id-peminjaman="item" v-if="itemOnHoverID===item.ID"/>
                  </v-dialog>
  ...
  props: {
    ...
    data(){
      return {
        ...
        breadcrumbItems: [
          {
            text: "Approval Peminjaman Arsip",
            disabled: false,
            href: "",
          },
        ],
        ...
        headers: [
          ...
          { text: "Nama Peminjam", value: "namaPeminjam", sortable: false },
          { text: "Kode Kantor Peminjam", value: "kodeKantorPeminjam", sortable: false },
          // { text: "Status Draft", value: "stDraft", sortable: false },
          { text: "Tgl Peminjaman", value: "tglPeminjaman", sortable: false },
      ...
      async doProcessApp(st_app){
        ...
        this.$swal({
          ...
          if (result.isConfirmed) {
            this.$axios.put(`/arsip/approvalPeminjaman`,{
                      "id": this.itemTemp.id,
    ...
    async deleteData(){
      ...
      async getListApproval() {
        ...
        this.$axios.get("/arsip/getListApprovalArsipPeminjaman",{
          params:{
              page:page,
              length:itemsPerPage,
              npk: this.dataUser.npk,
