    ...
    >
      ...
      <template v-slot:item="{ item }">
        <tr>
          ...
          <td v-if="item.stMasal!='Y'">{{ item.noNaskah }}</td>
          ...
          <td :class="item.fontStyle">{{ parseDate(item.tglTerima) }}</td>
          ...
          <td align="start">
              <!-- <v-btn :to="{ path: '/inbox/'+item.idNaskah }"
                class="mr-2" color="info" small >
                  <v-icon left>mdi-eye</v-icon>Lihat
            </v-btn> -->
            <v-tooltip left>
              <template v-slot:activator="{ on, attrs }">
                <v-icon v-if="item.stMasal=='Y' && parseInt(item.stProsesMasal)>0"
                  v-bind="attrs"
                  v-on="on"
            ...
            <!-- <v-btn v-if="parseInt(item.stProsesMasal)==0" class="mr-2" color="info" small @click="doDetilNaskahMasuk(item)" >
                  Lihat
            </v-btn> -->
            <v-btn icon v-if="parseInt(item.stProsesMasal)==0" @click="doDetilNaskahMasuk(item)"
                class="mr-2" color="info" small >
                  <v-icon>mdi-eye</v-icon>
            </v-btn>
            <v-btn class="mr-2" color="info" v-if="item.stMasal=='Y' && parseInt(item.stProsesMasal)==0" small @click="openDialog(item.idNaskah)" >
                MASSAL
            </v-btn>
          </td>
        </tr>
...
import masalFormDialogOutbox from "~/components/masalFormDialogOutbox";
export default {
  props: {
    filterTab: {
      type: String,
